...
$(document).ready(function () {
  ...
  $(".btn1").css("background", "#e71e10"); //첫번째 불켜
  ...
  function moveg() {
    ...
    $(".mbutton").css("background", "rgba(51, 51, 51, .5)"); //버튼불다꺼!!
    $(".mbutton").css("width", "16"); // 버튼 원래의 너비
    $(".btn" + cnt).css("background", "#e71e10"); //자신만 불켜
  ...
  $(".mbutton").click(function (event) {
    ...
    // for(var i=1;i<=imageCount;i++){
    //   $('.btn'+i).css('background','rgba(51, 51, 51, .5)'); //버튼 모두불꺼
    //   $('.btn'+i).css('width','16');
    // }
    $(".mbutton").css("background", "rgba(51, 51, 51, .5)"); //버튼 모두불꺼
    $(".mbutton").css("width", "16");
    $(".btn" + cnt).css("background", "#e71e10"); //자신 버튼만 불켜
    $(".btn" + cnt).css("width", "30");
  ...
  $(".ps").click(function () {
    if (onoff == true) {
      ...
      $(this).html('<i class="fas fa-play"></i>'); //js파일에서는 경로의 기준이 html파일이 기준!!
      onoff = false;
    } else {
      // false 타이머가 중지 상태냐??
